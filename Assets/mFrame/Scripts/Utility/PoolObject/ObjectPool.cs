    ...
    public sealed class ObjectPoolManager : Singleton<ObjectPoolManager>
    {
        private class ObjectPool
        {
            ...
            public void CloseObj(IPoolObject ir)
            {
                ...
                ir.myTran.gameObject.SetActive(false);
                ir.DormancyObj();
                ir.isUse = false;
            }
        ...
        public IEnumerator PreCreate(GameObject prefab, bool isCreate, int minCount = 1, int maxCount = int.MaxValue, Transform parent = null)
        {
            ...
            if (isCreate)
            {
                for (int i = 0; i < minCount; i++)
                {
                    ...
                    PreCreateObj(prefab.name, ir.myTran.gameObject, objList, parent);
                    yield return 0;
                }
            }
        }
        ...
        public void PreCreate(string prefabName, bool isCreate, int maxCount = int.MaxValue, Transform parent = null, bool isResource = false)
        {
            ...
            if (isCreate)
            {
                if (!isResource)
                {
                    AssetsManagerFor5.Instance.AddAssetTask(prefabName + ".prefab", delegate(UnityEngine.Object obj, object data)
                    {
                ...
                else
                {
                    try
                    {
                        AssetsManagerFor5.Instance.AddAssetTaskForAssetPath(prefabName + ".prefab", delegate(UnityEngine.Object obj, object data)
                        {
                            PreCreateObj(prefabName, obj, objList, parent);
                        }).allowMultiPrefabInstances = true;
        ...
        public IPoolObject Acquire(GameObject prefab, Transform parent = null)
        {
            ...
            if (ret != null)
            {
                if (null != parent)
                {
                    ret.myTran.parent = parent;
                }
                ret.myTran.gameObject.SetActive(true);
                return ret;
            }
            ...
            if (objList.GetAvailableCount() > 1)
            {
                ...
                AddObjToPool(ir.myTran.gameObject, objList);
                return ir;
            }
            return null;
        }
