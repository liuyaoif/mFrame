...
namespace mFrame.PathFinder
{
    public class AStarPathFinder
    {
        public delegate void OnPathFound(List<Vector2I> path);
        ...
        private List<Vector2I> m_neiborTempList = null;
        ...
        public AStarPathFinder(Vector2I size, List<Vector2I> cells)
        {
            m_size = size;
            ...
            m_neiborTempList = new List<Vector2I>(8);
        ...
        public void AddPathFindingTask(Vector2I from, Vector2I to, OnPathFound callBack)
        {
            lock (m_newTaskQueue)
            {
        ...
        private List<Vector2I> FindPath(Vector2I start, Vector2I end)
        {
            ...
            while (m_openList.Count != 0)
            {
                ...
                {
                    Vector2I point = m_neiborTempList[i];
        ...
        private void NotFoundPoint(ASNode tempStart, Vector2I end, Vector2I point)
        {
            ASNode node = m_allNodesDict[point.guid];
        ...
        private int CalcH(Vector2I end, ASNode node)
        {
        ...
        private void GetNeiborPoints(ASNode point)
        {
            for (int x = point.pos.x - 1; x <= point.pos.x + 1; x++)
            {
                ...
                {
                    if (IsInRange(x, y) && !m_closeList.ContainsKey(Vector2I.CalcSingleValue(x, y)))
                    {
                        m_neiborTempList.Add(new Vector2I(x, y));
                    }
                }
            }
        }
        ...
        {
            public Vector2I m_fromPos = Vector2I.minusOne;
            public Vector2I m_toPos = Vector2I.minusOne;
            public OnPathFound m_callBack = null;
            public List<Vector2I> m_path = null;

            public PathFindingTask(Vector2I from, Vector2I to, OnPathFound callBack)
            {
                m_fromPos = from;
                m_toPos = to;
                m_callBack = callBack;
